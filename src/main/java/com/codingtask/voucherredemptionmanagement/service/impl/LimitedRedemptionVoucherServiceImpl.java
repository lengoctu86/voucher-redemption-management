...
import com.codingtask.voucherredemptionmanagement.exception.custom.VoucherExpiredException;
...
import com.codingtask.voucherredemptionmanagement.service.validator.LimitedRedemptionVoucherValidator;
...
@Transactional
@AllArgsConstructor
public class LimitedRedemptionVoucherServiceImpl extends LimitedRedemptionVoucherValidator implements VoucherService {

    private final VoucherRepository voucherRepository;

    @Override
    public VoucherDTO createVoucher(VoucherRequestDTO dto) {
        validateLimitedRedemptionVoucher(dto);
    ...
    public VoucherDTO redeemVoucher(RedemptionDTO dto) {
        ...
        if (LocalDateTime.now().isAfter(existingVoucher.getExpiryDate()) ) {
            existingVoucher.setStatus(VoucherStatus.EXPIRED);
